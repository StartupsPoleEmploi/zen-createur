...
function calculateTotal(employers, field) {
	const sum = employers.reduce((prev, employer) => parseFloat(employer[field]) + prev, 0);
...
const COLUMNS_EMPLOYERS = [
	...
	{ title: 'Statut', dataIndex: 'status', ...defaultVal },
	{ title: 'CA', dataIndex: 'turnover', ...defaultVal },
	{ title: 'Temps travailler', dataIndex: 'workHours', ...defaultVal },
...
export default function DeclarationEnterprises({ declaration }: Props) {
	const workHours = calculateTotal(declaration.revenues, 'workHours');
	const salary = calculateTotal(declaration.revenues, 'turnover');
	const totalStr = `Total: ${workHours} h, ${salary} €`;

	const data = declaration.revenues.map((employer) => ({
		...employer,
	...
	return (
		<Card title="Créateur" style={{ marginBottom: '20px' }} extra={<h3>{totalStr}</h3>}>
			<ZnTable
				rowKey="id"
				size="small"
