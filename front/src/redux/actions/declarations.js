...
export const fetchDeclarations = ({ limit } = {}) => (dispatch) => {
  ...
  fetchDeclarationsPromise = superagent
    ...
    .then((res) => dispatch({ type: FETCH_DECLARATIONS_SUCCESS, payload: res.body }))
...
const getUploadErrorMessage = (err) => (err.status === 413 ?
...
export const removeEmployerFilePage = ({
  ...
  return superagent
    ...
    .then((res) => dispatch({ type: FETCH_EMPLOYER_SUCCESS, payload: res.body }))
    .catch(catchMaintenance)
    .catch((err) => {
      if (manageErrorCsrfToken(err, dispatch)) return;
      dispatch({
...
export const postEmployers = (formData) => (dispatch, getState) => superagent
